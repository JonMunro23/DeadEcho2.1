...
namespace GameBuilders.FPSBuilder.Core.Inventory
{
    ...
    public sealed class WeaponManager : MonoBehaviour
    {
        ...
        ///// <summary>
        ///// Defines the reference to the UIController script.
        ///// </summary>
        //[SerializeField]
        //[Required]
        //[Tooltip("Defines the reference to the UIController script.")]
        //private FirstPersonCharacterController m_UIController;
        ...
        /// <summary>
        /// Determines the GameObject tag identifier for ammo pickups.
        /// </summary>
        [SerializeField]
        [Tag(AllowUntagged = false)]
        [Tooltip("Determines the GameObject tag identifier for ammo pickups.")]
        private string m_AmmoTag = "Ammo";

        /// <summary>
        /// Determines the GameObject tag identifier for doors.
        /// </summary>
        [SerializeField]
        [Tag(AllowUntagged = false)]
        [Tooltip("Determines the GameObject tag identifier for doors.")]
        private string m_DoorTag = "Door";

        /// <summary>
        /// Determines the GameObject tag identifier for adrenaline packs pickups.
        /// </summary>
        [SerializeField]
        [Tag(AllowUntagged = false)]
        ...
        /// <summary>
        /// The GameObject tag used for doors.
        /// </summary>
        public string DoorTag => m_DoorTag;
        ...
        {
            PowerUpManager.onGiveMaxAmmo += OnPickedUpMaxAmmo;
        }
                ...
                if (target)
                {
                    ...
                    if (m_CurrentWeapon != null)
                    {
                        ...
                        if (IsEquipped(weapon))
                        {
                            ...
                            {
                                if(PointsManager.Instance.CurrentPoints >= PointsManager.Instance.AmmoCost)
                                {
                                    PointsManager.Instance.PurchaseItem(PointsManager.Instance.AmmoCost);
                                    StartCoroutine(RefillTargetWeaponAmmo());
                                }
                            }
                            return;
                        }
                        ...
                        if (HasFreeSlot)
                        {
                            if (m_InteractAction.triggered)
                            {
                                if(PointsManager.Instance.CurrentPoints >= weapon.WeaponCost)
                                {
                                    PointsManager.Instance.PurchaseItem(weapon.WeaponCost);
                ...
                && m_CurrentWeapon.CanUseEquipment && CanRefillAmmo())
            {
                if (Target)
                {
                    ...
                    if (Target.CompareTag(m_AmmoTag))
                    {
                        if (m_InteractAction.triggered)
                        {
                            StartCoroutine(RefillAllAmmo(false));
                        }
                    }
                }
            }
        ...
        void OnPickedUpMaxAmmo()
        {
            Debug.Log("Started Refill Amoo coroutine");
            StartCoroutine(RefillAllAmmo(true));
        }

        /// <summary>
        /// Refills the character magazines for all equipped weapons.
        /// </summary>
        private IEnumerator RefillAllAmmo(bool wasPowerUp)
        {
            m_ItemCoolDown = true;
            ...
            for (int i = 0, c = m_EquippedWeaponsList.Count; i < c; i++)
            {
                ...
                m_EquippedWeaponsList[i].SetCurrentRounds(m_EquippedWeaponsList[i].GunData.HasChamber ? m_EquippedWeaponsList[i].GunData.RoundsPerMagazine + 1 : m_EquippedWeaponsList[i].GunData.RoundsPerMagazine);
            }
        ...
        /// Refills the character magazines for all equipped weapons.
        /// </summary>
        private IEnumerator RefillTargetWeaponAmmo()
        {
            var target = Target;
            m_ItemCoolDown = true;
            ...
            for (int i = 0, c = m_EquippedWeaponsList.Count; i < c; i++)
            {
                ...
                if (m_EquippedWeaponsList[i].GunData.GunName == target.GetComponent<GunPickup>().weaponName)
                {
                    AmmoInstance ammoInstance = GetAmmoInstance(m_EquippedWeaponsList[i].GunData.AmmoType);
                    ammoInstance.Amount = ammoInstance.MaxAmount;
                    m_EquippedWeaponsList[i].SetCurrentRounds(m_EquippedWeaponsList[i].HasChamber ? m_EquippedWeaponsList[i].RoundsPerMagazine + 1 : m_EquippedWeaponsList[i].RoundsPerMagazine);
                    break;
                }
            }
        ...
        //void DropAndChange2(IWeapon current, IWeapon target, GunPickup drop, int currentRounds)
        //{
        //    current.Deselect();
        //    //if (!m_FastChangeWeapons)
        //    //{
