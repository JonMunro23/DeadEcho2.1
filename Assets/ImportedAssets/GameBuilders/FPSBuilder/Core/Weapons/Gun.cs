...
namespace GameBuilders.FPSBuilder.Core.Weapons
{
    ...
    public class Gun : MonoBehaviour, IWeapon
    {
        ...
        /// </summary>
        public int WeaponCost => m_GunData != null ? m_GunData.WeaponCost : 0;
        ...
        private void Update()
        {
            ...
            m_Accuracy = Mathf.Clamp(Mathf.MoveTowards(m_Accuracy, GetCurrentAccuracy(), Time.deltaTime *
                                    (m_IsShooting > 0 ? (IsAiming ? m_GunData.DecreaseRateByShootingWhileAiming : m_GunData.DecreaseRateByShooting) : m_GunData.DecreaseRateByWalking)),
        ...
        private void HandleInput()
        {
            ...
            bool canShoot = !m_IsReloading && m_NextReloadTime < Time.time && m_NextFireTime < Time.time && (CurrentRounds >= 0 || PowerUpManager.isBottomlessClipActive)
            ...
            if (m_GunData.ReloadType == GunData.ReloadMode.BulletByBullet && m_IsReloading && m_NextReloadTime < Time.time && (CurrentRounds > (m_GunData.HasChamber ? 1 : 0) || (CurrentRounds == 0 && m_InventoryManager.GetAmmo(m_GunData.AmmoType) > 0) || PowerUpManager.isBottomlessClipActive))
            {
            ...
            if (canShoot)
            {
                if (FireMode == GunData.FireMode.FullAuto || FireMode == GunData.FireMode.ShotgunAuto)
                {
                    ...
                    {
                        if ((CurrentRounds == 0 && m_InventoryManager.GetAmmo(m_GunData.AmmoType) > 0) && !PowerUpManager.isBottomlessClipActive)
                        {
                            Reload();
                        }
                        else
        ...
        private void PullTheTrigger()
        {
            if ((CurrentRounds > 0 && m_InventoryManager.GetAmmo(m_GunData.AmmoType) >= 0) || PowerUpManager.isBottomlessClipActive)
            {
                ...
                {
                    m_NextFireTime = Time.time + (m_FireInterval - (m_FireInterval * PlayerUpgrades.fireRateModifier));
                ...
                {
                    m_NextFireTime = Time.time + (m_FireInterval - (m_FireInterval * PlayerUpgrades.fireRateModifier)) * (m_GunData.BulletsPerBurst + 1);
        ...
        private IEnumerator Burst()
        {
            ...
            {
                if (CurrentRounds == 0 && !PowerUpManager.isBottomlessClipActive)
                    break;
        ...
        private void Shot()
        {
            ...
            if (Physics.Raycast(ray, out RaycastHit hitInfo, m_GunData.Range, m_GunData.AffectedLayers, QueryTriggerInteraction.Collide))
            {
                ...
                { 
                    damage = damage + PlayerUpgrades.weaponDamageModifier;
                    IProjectileDamageable damageableTarget;
                    if (hitInfo.transform.root.CompareTag("Zombie"))
                    {
                        if(hitInfo.transform.CompareTag("ZombieHead"))
                        {
                            damage *= 2 + PlayerUpgrades.bonusheadshotMultiplier;
                        }
                        damageableTarget = hitInfo.collider.GetComponentInParent<IProjectileDamageable>();
                    }
                    else
                        damageableTarget = hitInfo.collider.GetComponent<IProjectileDamageable>();

                    //Debug.Log("Hit " + hitInfo.transform.tag);
                    //Debug.Log(damage);
                    damageableTarget?.ProjectileDamage(hasSplintered ? damage * m_GunData.AmmoType.FragmentationDamageMultiplier : damage, transform.root.position, hitInfo.point, m_GunData.AmmoType.PenetrationPower, hitInfo.transform.tag);
                }
        ...
        private IEnumerator MeleeAttack()
        {
            ...
            Vector3 origin = m_CameraTransformReference.transform.position;
            float range = m_GunData.Size * 0.75f + m_FPController.Radius;
            ...
            if (Physics.Raycast(ray, out RaycastHit hitInfo, range, m_GunData.AffectedLayers, QueryTriggerInteraction.Collide))
            {
                ...
                if (hitInfo.transform.root != transform.root)
                {
                    IMeleeDamageable damageableTarget;
                    if (hitInfo.transform.root.CompareTag("Zombie"))
                    {
                        damageableTarget = hitInfo.collider.GetComponentInParent<IMeleeDamageable>();
                    }
                    else
                        damageableTarget = hitInfo.collider.GetComponent<IMeleeDamageable>();

                    damageableTarget?.MeleeDamage(m_GunData.MeleeDamage, transform.root.position, hitInfo.point, hitInfo.collider.tag);
                }
        ...
        private IEnumerator ReloadMagazines()
        {
            ...
            m_CompleteReloadDuration = new WaitForSeconds(m_GunAnimator.FullReloadAnimationLength - m_GunAnimator.FullReloadAnimationLength * PlayerUpgrades.reloadSpeedModifier);
            m_ReloadDuration = new WaitForSeconds(m_GunAnimator.ReloadAnimationLength - m_GunAnimator.ReloadAnimationLength * PlayerUpgrades.reloadSpeedModifier);
        ...
        private IEnumerator ReloadBulletByBullet()
        {
            ...
            m_InsertInChamberDuration = new WaitForSeconds((m_GunAnimator.InsertInChamberAnimationLength / 2) - ((m_GunAnimator.InsertInChamberAnimationLength / 2) * PlayerUpgrades.reloadSpeedModifier));
            m_StartReloadDuration = new WaitForSeconds(m_GunAnimator.StartReloadAnimationLength - m_GunAnimator.StartReloadAnimationLength * PlayerUpgrades.reloadSpeedModifier);
            m_InsertDuration = new WaitForSeconds((m_GunAnimator.InsertAnimationLength / 2) - ((m_GunAnimator.InsertAnimationLength / 2) * PlayerUpgrades.reloadSpeedModifier));
        ...
        {
            m_StopReloadDuration = new WaitForSeconds(m_GunAnimator.StopReloadAnimationLength - m_GunAnimator.StopReloadAnimationLength * PlayerUpgrades.reloadSpeedModifier);

            m_GunAnimator.StopReload();
            m_IsReloading = false;
            m_NextReloadTime = (m_GunAnimator.StopReloadAnimationLength - m_GunAnimator.StopReloadAnimationLength * PlayerUpgrades.reloadSpeedModifier) + Time.time;
