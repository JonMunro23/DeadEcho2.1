    ...
    public static class Explosion
    {
        ...
        public static void CalculateExplosionDamage(float radius, float force, float damage, Vector3 position, bool ignoreCover = false)
        {
            ...
            for (int i = 0; i < amount; i++)
            {
                ...
                if (hitZombies.Contains(c.transform.root.gameObject))
                {
                    Debug.Log("Contains gameobject, continuing...");
                    continue;
                }
                ...
                IExplosionDamageable damageableTarget;
                if (c.transform.root.CompareTag("Zombie"))
                {
                    hitZombies.Add(c.transform.root.gameObject);
                    damageableTarget = c.GetComponentInParent<IExplosionDamageable>();
                }else
                    damageableTarget = c.GetComponent<IExplosionDamageable>();
