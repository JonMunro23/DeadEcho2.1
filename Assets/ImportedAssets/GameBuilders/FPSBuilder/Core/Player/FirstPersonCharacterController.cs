    ...
    public sealed class FirstPersonCharacterController : MonoBehaviour
    {
        ...
        private void ApplyInputVelocityChange(Vector2 input)
        {
            ...
            else
            {
                if (Mathf.Abs(input.x) - Mathf.Epsilon > 0 || Mathf.Abs(input.y) - Mathf.Epsilon > 0)
                {
                    ...
                    float currentTargetForceModified = CurrentTargetForce + CurrentTargetForce * PlayerUpgrades.moveSpeedModifier;

                    desiredMove.x *= (Grounded ? currentTargetForceModified : currentTargetForceModified * m_AirControlPercent);
                    desiredMove.z *= (Grounded ? currentTargetForceModified : currentTargetForceModified * m_AirControlPercent);
                    desiredMove.y = desiredMove.y * (Grounded ? currentTargetForceModified : currentTargetForceModified * m_AirControlPercent);

                    if (m_RigidBody.velocity.sqrMagnitude < (currentTargetForceModified * currentTargetForceModified))
                    {
                        m_RigidBody.AddForce(desiredMove, ForceMode.Impulse);
                    }
                }
        ...
        private void ApplyGravityAndJumping(Vector2 input)
        {
            ...
            else
            {
                ...
                if (m_RigidBody.velocity.magnitude < Mathf.Abs(Physics.gravity.y * m_CharacterMass / 2))
                {
